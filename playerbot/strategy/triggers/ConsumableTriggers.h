...
//    };
   ...
   inline void CreateBuffToConsumableUseTrigger(std::map<std::string, std::function<Trigger*(PlayerbotAI*)>>& creators, uint32 itemId, uint32 buffId)
   {
      std::ostringstream ss;
   ...
   inline void CreateUseFlaskAction(std::map<std::string, std::function<Action* (PlayerbotAI*)>>& creators)
   {
      creators["use flask"] = [](PlayerbotAI* ai) -> Action*
         {
            ...
            return new UseAction(ai, itemlink.str().c_str(), true);
         };
   }

   inline void CreateUseConsumableAction(std::map<std::string, std::function<Action* (PlayerbotAI*)>>& creators, uint32 itemId)
   {
      std::ostringstream ss;
      ss << "use potion " << itemId;
